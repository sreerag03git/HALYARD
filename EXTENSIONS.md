...
| **6-DOF platform (add heave, sway, yaw)** | Deferred: surge + pitch capture the dominant, pitch-driven hang-off bending fatigue (the control-sensitive mechanism). Heave chiefly drives hang-off *tension* variation; its omission is a stated limitation (the tension-range contribution is under-represented, which is conservative for the *relative* control effect). |
