...
| **DLC fatigue matrix + multi-seed UQ** (major) | Reduced IEC-DLC-1.2-style matrix: wind speeds (Weibull) × wind-correlated sea state × wave headings × seeds; annualized hang-off life with a 5–95% band from seed scatter + S-N log-N scatter. | IEC 61400-1 DLC 1.2 | button (offline) | Fatigue tab |
| **S-N scatter propagation** | S-N log10(N) standard deviation (~0.20) propagated log-normally to the life band. | DNV-RP-C203 scatter | on (in the DLC band) | — |
...
| **Real IEA-15MW rotor deck** | The public OpenFAST Cp_Ct_Cq deck (36 pitch × 26 TSR) bundled and loaded verbatim; steeper, realistic dCt/dλ. | IEAWindTask37/IEA-15-240-RWT | **on** (bundled) | falls back to calibrated surfaces if removed |
| **Blade-pitch control (ROSCO-style)** | Collective-pitch PI on rotor-speed error (active above rated) + floating (nacelle-velocity) feedback for aero damping; actuator rate/lag; a pitch state. Enables above-rated wind (sheds thrust) and the pitch–support interaction. | ROSCO; floating-feedback control | **on** | `blade_pitch`; wind slider to 24 m/s |
| **Turbulent inflow (Kaimal)** | IEC 61400-1 longitudinal Kaimal spectrum hub-height wind, seeded. | IEC 61400-1 | **off** (isolates the control effect) | turbulence-intensity slider |
| **Dynamic inflow lag** | First-order lag (τ≈4 s) on the rotor thrust used to force the platform (unsteady induction). | Øye / dynamic-inflow | **on** | `dynamic_inflow` |
| **Dynamic FE cable + VIV** (major) | Lumped-mass dynamic cable driven by the fairlead motion: Morison inertia+drag (wave kinematics per node), bending (EI), seabed contact+Coulomb friction, clamped bend-stiffener top BC rotating with platform pitch, inextensibility via red-black PBD projection (stable at the sim dt), dynamic tension from the moving geometry, and cross-flow VIV via a van der Pol wake oscillator. Gives dynamic amplification SELF-CONSISTENTLY (vs an assumed DAF) — typically a larger hang-off stress range than quasi-static. | Morison; Facchinetti et al. (2004) wake oscillator; PBD | off (button; ~8 s, cached) | Platform & cable tab; VIV checkbox |
...
| ~~Turbulent inflow~~ | **NOW IMPLEMENTED** (Kaimal, toggle) — off by default so the control-vs-wave comparison stays clean; on, it drives realistic broadband platform motion. |
| ~~Blade-pitch control above rated~~ | **NOW IMPLEMENTED** (ROSCO-style + floating feedback) — the wind range now extends to 24 m/s. |
...
| ~~Vortex-induced vibration (VIV)~~ | **NOW IMPLEMENTED** in the dynamic FE cable (van der Pol wake oscillator, cross-flow). |
| ~~Multi-seed UQ with confidence bands~~ | **NOW IMPLEMENTED** in the DLC matrix (multiple seeds per condition -> a seed-scatter band, combined with the S-N log-N scatter). |
